...
setTimeout(() => {
  (async () => {
    ...
    // Startup migration: normalized Investment Company product criteria.
    setTimeout(async () => {
      try {
        const { pool } = await import('./db');
        await pool.query(`
          CREATE TABLE IF NOT EXISTS developer_product_types (
            id VARCHAR PRIMARY KEY DEFAULT gen_random_uuid(),
            developer_profile_id VARCHAR NOT NULL REFERENCES developer_profiles(id) ON DELETE CASCADE,
            name VARCHAR NOT NULL,
            min_acres DECIMAL NOT NULL,
            max_acres DECIMAL,
            min_rent_psf DECIMAL,
            min_rent_per_unit DECIMAL,
            is_active BOOLEAN NOT NULL DEFAULT TRUE,
            created_at TIMESTAMP DEFAULT NOW(),
            updated_at TIMESTAMP DEFAULT NOW()
          );
          CREATE INDEX IF NOT EXISTS developer_product_types_profile_idx
            ON developer_product_types (developer_profile_id);
          CREATE UNIQUE INDEX IF NOT EXISTS developer_product_types_profile_name_unique
            ON developer_product_types (developer_profile_id, lower(name));

          CREATE TABLE IF NOT EXISTS developer_county_market_labels (
            id VARCHAR PRIMARY KEY DEFAULT gen_random_uuid(),
            developer_profile_id VARCHAR NOT NULL REFERENCES developer_profiles(id) ON DELETE CASCADE,
            county VARCHAR NOT NULL,
            market_label VARCHAR NOT NULL,
            created_at TIMESTAMP DEFAULT NOW(),
            updated_at TIMESTAMP DEFAULT NOW()
          );
          CREATE UNIQUE INDEX IF NOT EXISTS developer_county_market_labels_profile_county_unique
            ON developer_county_market_labels (developer_profile_id, county);
          CREATE INDEX IF NOT EXISTS developer_county_market_labels_profile_idx
            ON developer_county_market_labels (developer_profile_id);

          ALTER TABLE partner_developer_sends
            ADD COLUMN IF NOT EXISTS matched_product_types TEXT[];

          INSERT INTO developer_product_types (
          ...
          WHERE (
            p.min_acres IS NOT NULL
            OR p.min_rent_psf IS NOT NULL
            OR p.min_rent_per_unit IS NOT NULL
          )
          AND NOT EXISTS (
            SELECT 1
            FROM developer_product_types pt
            WHERE pt.developer_profile_id = p.id
          );
        `);
        console.log('✅ Investment Company product-type criteria migration ready');
      } catch (err: any) {
        console.error('❌ Investment Company product-type criteria migration error:', err.message);
      }
    }, 4500);

...
export default function DealDetails() {
  ...
  const generateDealSummaryPdf = async () => {
    ...
    try {
      const response = await fetch(`/api/deals/${encodeURIComponent(id)}/memo-pdf`, {
        credentials: "include",
      });
      if (!response.ok) {
        const payload = await response.json().catch(() => ({}));
        throw new Error(payload.message || "Unable to generate the Deal Memo");
      }
      const blob = await response.blob();
      const disposition = response.headers.get("Content-Disposition") || "";
      const filenameMatch = disposition.match(/filename="([^"]+)"/i);
      const filename = filenameMatch?.[1] || "Deal-Memo.pdf";

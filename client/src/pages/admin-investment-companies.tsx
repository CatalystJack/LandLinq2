...
export default function AdminInvestmentCompanies() {
  ...
  return <div className="min-h-screen bg-slate-50">
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="text-sm font-semibold uppercase tracking-wider text-blue-600">Platform administration</p><h1 className="mt-1 text-3xl font-bold text-slate-950">Investment Companies</h1><p className="mt-2 text-slate-600">Create company portals, configure acquisition criteria, and issue the first login.</p></div><Button onClick={openCreate}><Plus className="mr-2 h-4 w-4" />Create New Investment Company</Button></div>
      {companiesQuery.isLoading ? <div className="flex justify-center py-24"><Loader2 className="h-8 w-8 animate-spin text-blue-600" /></div> : companiesQuery.isError ? <Card><CardContent className="py-12 text-center text-red-600">Unable to load Investment Company profiles.</CardContent></Card> : !sortedProfiles.length ? <Card className="border-dashed"><CardContent className="flex flex-col items-center py-16 text-center"><Building2 className="mb-4 h-12 w-12 text-slate-300" /><h2 className="text-xl font-semibold">No Investment Companies yet</h2><p className="mt-2 max-w-md text-slate-500">Create the first profile when your team is ready. No company records are created automatically.</p><Button className="mt-6" onClick={openCreate}><Plus className="mr-2 h-4 w-4" />Create profile</Button></CardContent></Card> :
    ...
    <Dialog open={formOpen} onOpenChange={setFormOpen}><DialogContent className="max-h-[92vh] max-w-4xl overflow-y-auto"><DialogHeader><DialogTitle>{editing ? `Manage ${editing.companyName}` : "Create New Investment Company"}</DialogTitle><DialogDescription>Branding, acquisition criteria, targeting, and portal availability.</DialogDescription></DialogHeader>
      ...
      </div><DialogFooter><Button variant="outline" onClick={() => setFormOpen(false)}>Cancel</Button><Button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending || logoMutation.isPending}>{saveMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{editing ? "Save changes" : "Create Investment Company"}</Button></DialogFooter>
    </DialogContent></Dialog>

    <Dialog open={!!loginCompany} onOpenChange={(open) => !open && setLoginCompany(null)}><DialogContent><DialogHeader><DialogTitle>Create Initial Login</DialogTitle><DialogDescription>Create the first developer account for {loginCompany?.companyName}. A temporary password and branded login link will be emailed to the contact.</DialogDescription></DialogHeader><div className="space-y-4 py-3"><div><Label>Contact name</Label><Input value={contactName} onChange={(e) => setContactName(e.target.value)} placeholder="First and last name" /></div><div><Label>Email</Label><Input type="email" value={contactEmail} onChange={(e) => setContactEmail(e.target.value)} placeholder="contact@company.com" /></div><div className="flex gap-2 rounded-lg bg-blue-50 p-3 text-sm text-blue-900"><Mail className="mt-0.5 h-4 w-4 shrink-0" /><p>The account will be assigned the DEVELOPER role and must choose a new password at first sign-in.</p></div></div><DialogFooter><Button variant="outline" onClick={() => setLoginCompany(null)}>Cancel</Button><Button onClick={() => loginMutation.mutate()} disabled={loginMutation.isPending}>{loginMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Create and send login</Button></DialogFooter></DialogContent></Dialog>
  </div>;
}